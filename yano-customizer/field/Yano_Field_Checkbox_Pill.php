...
use Control\Yano_Checkbox_Pill_Control;
 ...
 * @version 1.0.0
 */
final class Yano_Field_Checkbox_Pill extends Yano_Settings {
	...
	public function render( $wp_customize, $config ) {
		...
		$field_name =  yano_error_field_name( 'checkbox-pill', $config['id'] );
		...
		if ( is_array( $args ) && parent::sanitize_argument( $config, $field_name ) != false && $is_valid_default == true ) {
			...
			$wp_customize->add_control( new Yano_Checkbox_Pill_Control( $wp_customize, $args['id'] . '_field', array(
