...
use Control\Yano_Dropdown_Page_Control;
 ...
 * @version 1.0.0
 */
final class Yano_Field_Dropdown_Page extends Yano_Settings {
	...
	public function render( $wp_customize, $config ) {
		...
		$field_name =  yano_error_field_name( 'dropdown-page', $config['id'] );
		...
		if ( is_array( $args ) && parent::sanitize_argument( $config, $field_name ) != false && $is_valid_order == true ) {
			...
			$wp_customize->add_control( new Yano_Dropdown_Page_Control( $wp_customize, $args['id'] . '_field', array(
