...
use Control\Yano_Image_Checkbox_Control;
 ...
 * @version 1.0.0
 */
final class Yano_Field_Image_Checkbox extends Yano_Settings {


	/**
	 * Rendering Image Checkbox.
	 *
	 * @since 1.0.0
	 * 
	...
	public function render( $wp_customize, $config ) {
		...
		$field_name =  yano_error_field_name( 'image-checkbox', $config['id'] );
		...
		if ( is_array( $args ) && parent::sanitize_argument( $config, $field_name ) != false && $is_valid_choices == true && $is_valid_direction == true && $is_valid_size == true && $is_valid_default == true ) {
			...
			$wp_customize->add_control( new Yano_Image_Checkbox_Control( $wp_customize, $args['id'] . '_field', array(
