...
use Control\Yano_Switch_Control;
 ...
 * @version 1.0.0
 */
final class Yano_Field_Switch extends Yano_Settings {
	...
	public function render( $wp_customize, $config ) {
		...
		$field_name =  yano_error_field_name( 'switch', $config['id'] );
		...
		if ( is_array( $args ) && parent::sanitize_argument( $config, $field_name ) != false ) {
			...
			$wp_customize->add_control( new Yano_Switch_Control( $wp_customize, $args['id'] . '_field', array(
