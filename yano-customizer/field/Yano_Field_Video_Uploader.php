...
use Control\Yano_Video_Uploader_Control;
 ...
 * @version 1.0.0
 */
final class Yano_Field_Video_Uploader extends Yano_Settings {
	...
	public function render( $wp_customize, $config ) {
		...
		$field_name =  yano_error_field_name( 'video-uploader', $config['id'] );
		...
		if ( is_array( $args ) && parent::sanitize_argument( $config, $field_name ) != false && $is_valid_extensions == true ) {
			...
			$wp_customize->add_control( new Yano_Video_Uploader_Control( $wp_customize, $args['id'] . '_field', array(
